...
describe('Chat App new-session flow', () => {
  ...
  it('codex prompt payload omits `model` if sent before catalog seeds selection', async () => {
    // Regression: a fresh Codex session that races ahead of
    // `catalog.providers()` would otherwise ship FALLBACK_SELECTION's
    // OpenCode-shaped model id (anthropic / claude-sonnet-4-...) which
    // Codex would reject as a user-visible send failure.
    setRouteParams({
      isNewSession: true,
      connection: {
        kind: 'codex',
    ...
    const subscription = createSubscription()
    const promptMock = vi.fn(async () => ({ status: 'sent' }))
    // `providers` never resolves during the test — selection stays at
    // FALLBACK_SELECTION because the catalog seeding effect can't fire.
    const codexClient: BackendClient = {
      descriptor: { kind: 'codex', label: 'Codex' },
      capabilities: {
        sessions: true,
    ...
    // Catalog never resolves; tap the composer to fire send before any
    // catalog-driven seeding can happen.
    const composer = result.container.querySelector('.composer-stub')
    expect(composer).not.toBeNull()
    fireEvent.tap(composer!)
    ...
    const firstCall = promptMock.mock.calls[0] as unknown as [string, Record<string, unknown>]
    const payload = firstCall[1]
    expect(payload.parts).toEqual([{ type: 'text', text: 'hello world' }])
    // The fix: model omitted entirely while selection is uninitialized.
